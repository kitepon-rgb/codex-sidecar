...
Status: 緊急コンテキスト緩和完了・本修正契約は反証通過・Slice 0/1 実装中
作成日: 2026-07-12  
...
- [ ] spawn後ready前crashのcharacterizationを追加する。
- [x] config drift後same-key retryのcharacterizationを追加する。
- [ ] spawn→spawn record間publisher crashのcharacterizationを追加する。
- [x] linked worktree path conflictとHEAD移動後retry/固定OID executionを追加する。
- [ ] auth lease claim直後/clean直後crash recoveryを追加する。

### Slice 1 — store/public types

- [x] run tagged unionとrun error codesをcoreに追加する。
- [ ] start/result/cancel/recoveryのtool別input contractをcoreに追加する。
