...
- `result.json`が有効なら、terminal marker欠落時もrecord内の耐久`terminalState`
  （`completed`/`failed`/`cancelled`）へ回復し、interruptedへ降格しない。新規writerは必ず
  `terminalState`をresultと同じcreate-only recordへ固定し、pollがterminalを先取しても
  cancelの最終状態を取り違えない。旧recordにfieldが無い場合だけ既存のresult statusから導出する。
...
current generationをfenceする。これは`terminal.json`を偽造せず、以後のpollは
...
`auth/app-server-started.json`が存在する場合は、worker process groupが停止済みでもquarantineを拒否し、
exact runの`work-auth-recover` inspectionへ戻す。auth消費済みかもしれないwrite-ahead境界を
terminal interruptionとして確定しない。
stuck transition leaseがある場合は、そのowner不在とtoken/inode不変を確認し、operator確認付き操作の
transition token固有の監査recordを先に残してexact current hard-linkだけを解除する。その後、自身のtransition leaseを取得して
...
- mutation成功時は、直前のinspectionを再実行して現在のglobal leaseを上書き表示せず、対象run・
  strategy・exact lease tokenを束縛した`work_auth_recovery_ack`を返す。後続の別runがleaseを取得しても、
  このackは完了済みのrecovery事実を失わない。最新状態が必要なcallerだけが別途read-only inspectionを行う。
...
├── operator-recovery-<transition-token>.json # dead transition解除ごとのcreate-only監査record、任意
├── result.json          # worker create-only、lossless SidecarResult + durable terminalState
...
- result作成後・terminal作成前のcrashでは、valid resultをrecord内の`terminalState`へ昇格する。
...
実施日: 2026-07-12（JST）
対象 artifact: local packed/global install の `codex-sidecar` 0.3.3、source revision
`a6f6c74df42826f8183ff5bde56054f813b432c8`
環境: macOS 26.5.1 (25F80) / Darwin 25.5.0 arm64 / codex CLI 0.144.1 /
Claude Code 2.1.207

次の記録は、実機 fixture の durable record と raw App Server JSONL から抽出した秘密を含まない
永続 evidence である。fixture には isolated `codex-home` の認証コピーが含まれ得るため、raw log/
worktree 自体はリポジトリへ保存しない。下記 runId、時刻、terminal/log SHA-256 を正本に残し、
fixture は検証後に削除する。

| Case | Kill 時点と復帰 | Durable outcome | Evidence fingerprint |
| --- | --- | --- | --- |
| 実 Codex App Server | `app-server-started.json` を確認し `terminal.json` 不在のまま、実 Codex App Server が `sleep 8` 実行中に MCP coordinator PID 12613 を `SIGKILL`。新しい CLI process で同じ caller-held key を poll し、まず nonterminal heartbeat、続いて同一 run を回収。 | runId `e44c6a5cf86edb2ef0fe4c141fbf55ba6761cc9ccb315043b94f5853937d1a3f`。started `2026-07-12T12:18:37.828Z`、terminal `2026-07-12T12:19:16.050Z`、`completed` / `ok` / worktree preserved。 | terminal `fbf9a18c97465d9b477bced8bef42682fa1886484ccd87ef78cfeb95bc9036da`、raw JSONL `2df75669423a5064ff51b0f626d4d591b7c3de0bfa4737c06b1d74d4661d2f5f` |
| 実 Claude Code stdio MCP | strict MCP config の Claude Code から `codex_work_start` を1回実行。`app-server-started.json` 後・terminal 前に Claude PID 14659 を `SIGKILL`。別 CLI で terminal を回収し、再起動した実 Claude Code から同じ key の `codex_work_result` を実行。 | runId `1caa36915548be83cd2b00afd58de4bf24b9aba41ce33071f19dba77755d1d8e`。started `2026-07-12T12:21:50.894Z`、terminal `2026-07-12T12:22:17.893Z`、`completed` / `ok` / worktree preserved。再起動 Claude の返答は同 runId と `state: completed`。 | terminal `44c286fac4940b9d9b8d7e37207a350cede1f525e0eddb59877a3822667d4bf0`、raw JSONL `0cd8dc9df5d1d2e3665fc0243f4cf5ccb735e75d4581785bf7b4dfaf269833a0` |

両 case の最終 active tree は不変で、global canonical-auth lease は read-only `auth-status` で
`{ "state": "available" }` を確認した。idempotency key 本文は transcript にのみ残る caller-owned
input なので、この正本へは保存しない。
...
- [x] `work-recover` read-only inspectionと明示quarantine/terminal recoveryを実装する。
- [x] tests: ready前即死、publisher死亡引継ぎ、worker SIGKILL後descendant消滅、別project auth直列化、
  rotated auth clean recovery、abnormal killで`RUN_AUTH_UNCERTAIN`/lease保持。
- [x] auth各write-ahead境界kill、未回転valid tokenのwrite-back拒否、cancel中auth待機をtestする。
...
- [x] 実git tests: allowed/deny、active tree不変、preserve false ordering、cleanup crash recovery。

### Slice 4 — MCP/CLI async tools（完了）

- [x] tool別Zod schema/parserと`codex_work_start/result/cancel`を追加する。
- [x] auth recovery 4 strategyをcore API/tool/CLIの同一enumにし、全4値と未知値のparser testを追加する。
- [x] start後disconnect非cancel、cancel ack、poll union、CLI exit codeを実装する。
- [x] stdio/HTTPを跨いだsame-key recovery integration testを追加する。
- [x] packed tarball/global installでworker entrypointとversion整合を検証する。

### Slice 5 — integration/review/docs

- [x] coordinator SIGTERM/SIGKILL/stdio close後、新processからresultを取得する。
- [x] spawn後・response配送前に切断し、same-key retryで同じrunIdを取得する。
- [x] real Codex App Server実行中にcoordinatorをkillし、worker完了/結果回収を確認する。
- [x] 実Claude Code stdio MCPでstartし、Claude Code終了・再起動後にsame-key resultを回収する。
- [x] worker killで自動salvage/cleanupが行われず、auth leaseが明示回復まで保持されることを確認する。
...
worker entrypoint/MCP module load/`work-start --dry-run`をすべて確認した。
