...
Status: 緊急コンテキスト緩和完了・本修正契約は反証通過・Slice 0 着手  
作成日: 2026-07-12  
...
- [ ] response配送前切断、spawn後ready前crash、同一key並行startのcharacterizationを先に追加する。
- [ ] config drift後same-key retry、spawn→spawn record間publisher crashのcharacterizationを追加する。
- [ ] linked worktree path conflict、HEAD移動後retry/固定OID execution、auth lease claim直後/clean直後crash recoveryを追加する。

### Slice 1 — store/public types

- [ ] run tagged union、run error codes、tool別input contractをcoreに追加する。
- [ ] caller-reproducible canonical raw input digestと、既存runではconfigを再loadしないlookup順を実装する。
- [ ] common-dir store、再送可能raw identity、winner固定base commit execution snapshotを分離する。
- [ ] git-common-dir resolver、deterministic runId、atomic directory publishを実装する。
- [ ] atomic no-replace record publish、launch claim/steal、generation fencing、result→terminal recoveryを実装する。
- [ ] cross-process run transition lease、ABA-safe launch reclaim、execution-start/cancel線形化を実装する。
- [ ] unit tests: key conflict、concurrent start、publisher death、temp書込途中kill、corrupt final拒否、
  terminal競合、result/terminal間crash。

### Slice 2 — worker/process/auth

- [ ] worker entrypoint、ready handshake、parent非接続logs、heartbeatを実装する。
- [ ] POSIX process group identity/stop/消滅確認を実装する。
- [ ] canonical path identity global auth lease、run-local Codex home、run固有auth write-backを実装する。
- [ ] immutable auth phase journalと4 strategyの`work-auth-recover` inspection/明示解除を実装する。
- [ ] OS process間global auth lease、spawn前write-ahead marker、atomic rotation observerを実装する。
- [ ] 全同期/read-only App Server callをdurable auth sessionへ移し、`auth-status/recover`を実装する。
...
- [ ] auth不存在identity、lease claim直後の`release-never-started`、clean直後の`release-clean`をtestする。

### Slice 3 — worktree durability

- [ ] worktree executionとcleanupを分離し、同期wrapperの挙動を保つ。
- [ ] async result/terminal commit後だけ`preserveWorktree=false` cleanupを実行する。
