...
| Workflow | 目的 | 書き込み | 主な出力 |
|---|---|---:|---|
| `codex_review` | diff / branch / patch のレビュー | なし | `findings`, `missingTests`, `residualRisks` |
| `codex_explore` | codebase 調査 | なし | `summary`, `fileReferences` |
| `codex_opinion` | 設計や方針への反対意見 | なし | `recommendation`, `objections`, `assumptions` |
| `codex_risk_check` | secrets / MCP / OAuth / hooks / Docker / CI の重点確認 | なし | `risks`, `sourceBoundaries` |
| `codex_work` | 小さな実装作業 | 隔離 worktree のみ | `changedFiles`, `tests`, `worktreePath` |
...
- [docs/PLAN.md](docs/PLAN.md): roadmap と ecosystem overlay。
