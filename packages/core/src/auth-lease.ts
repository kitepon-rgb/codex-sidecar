...
export interface AuthLeaseRecovery { strategy: RecoveryStrategy; confirmNoRunningProcesses: boolean; }
...
interface MarkerRecord { version: 1; kind: MarkerKind | "operator-recovery"; token: string; canonicalAuthPath: string; initialAuthHash?: string; finalAuthHash?: string; canonicalAuthHash?: string; authWrittenBackDigest?: string; strategy?: RecoveryStrategy; generation?: string; mutexToken?: string; targetClaimDigest?: string; createdAt: string; digest: string; }
...
type MutexBinding =
  ...
  | { operation: "recover"; recoveryStrategy: RecoveryStrategy; decisionDigest: string; markerKind: null; markerDigest: null; markerName: null }
...
type MutexIntent =
  ...
  | { operation: "recover"; target: MutexTarget; recoveryStrategy: RecoveryStrategy; decisionDigest: string }
...
async function releaseAuthLeaseUnlocked(context: Awaited<ReturnType<typeof leaseContext>>, lease: AuthLease): Promise<void> {
...
export async function recoverAuthLease(input: AuthLeaseInput, recovery: AuthLeaseRecovery): Promise<void> {
  ...
  if (await settleHighest(context, mutexDirectory, recovery.strategy) === "recovery-completed") return;
  let held: AuthLease;
  ...
  await assertRecoveryEvidence(target.lease, recovery.strategy);
  ...
  await withMutex(context, { operation: "recover", target, recoveryStrategy: recovery.strategy, decisionDigest }, async (barrier) => {
    const current = await readCurrent(context);
    ...
    await releaseAuthLeaseUnlocked(context, current);
  });
}

async function withMutex<T>(context: Awaited<ReturnType<typeof leaseContext>>, intent: MutexIntent, action: (barrier: MutexRecord) => Promise<T>): Promise<T> {
  const directory = join(context.leaseDirectory, "mutex"); await ensureDirectory(directory);
  await settleHighest(context, directory);
...
async function settleHighest(context: Awaited<ReturnType<typeof leaseContext>>, directory: string, expectedRecoveryStrategy?: RecoveryStrategy): Promise<MutexSettlement> {
  ...
  if (highest.claim.operation === "claim") { if (current && !exact) throw coded("RUN_AUTH_UNCERTAIN", "abandoned claim has foreign current"); }
    ...
    if (expectedRecoveryStrategy && highest.claim.recoveryStrategy !== expectedRecoveryStrategy) throw coded("RUN_AUTH_UNCERTAIN", "abandoned recovery strategy conflicts with requested strategy");
...
async function leaseContext(input: AuthLeaseInput): Promise<Omit<AuthLease, "currentPath" | "claimPath" | "token" | "createdAt"> & { cacheRoot: string }> {
  ...
  return { leaseDirectory: join(cacheRoot, "codex-sidecar", "auth-leases", pathHash), canonicalAuthPath, pathHash, owner: { ...input.owner, journalPath: await realpath(input.owner.journalPath) }, cacheRoot };
}
...
async function assertRecoveryEvidence(lease: AuthLease, strategy: RecoveryStrategy): Promise<void> {
  await assertRecoveryJournal(lease, strategy);
  if (strategy !== "release-clean") return;
  try {
    const written = await readPrivateJson(join(lease.owner.journalPath, "auth-written-back.json")); assertMarker(written, "auth-written-back");
...
function markerRecord(kind: MarkerKind | "operator-recovery", lease: AuthLease, strategy?: RecoveryStrategy): MarkerRecord { const body = { version: 1 as const, kind, token: lease.token, canonicalAuthPath: lease.canonicalAuthPath, ...(strategy ? { strategy } : {}), createdAt: new Date().toISOString() }; return { ...body, digest: digest(stable(body)) }; }
function recoveryMarker(lease: AuthLease, strategy: RecoveryStrategy, barrier: MutexRecord, target: MutexTarget): MarkerRecord { const body = { version: 1 as const, kind: "operator-recovery" as const, token: lease.token, canonicalAuthPath: lease.canonicalAuthPath, strategy, generation: barrier.generation, mutexToken: barrier.token, targetClaimDigest: target.claimDigest, createdAt: new Date().toISOString() }; return { ...body, digest: digest(stable(body)) }; }
...
function assertMutexShape(value: unknown, released: boolean): void {
  ...
  if (recover ? (value.recoveryStrategy !== "release-never-started" && value.recoveryStrategy !== "release-clean" || !/^[a-f0-9]{64}$/.test(String(value.decisionDigest))) : value.recoveryStrategy !== null || !recover && value.decisionDigest !== null) throw coded("RUN_AUTH_UNCERTAIN", "invalid auth lease recovery mutex binding");
